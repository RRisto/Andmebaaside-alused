...
SELECT eesnimi, perenimi FROM isik, klubi
...
SELECT Perenimi FROM isik 
WHERE EXISTS(SELECT* FROM klubi 
...
SELECT Valge, Must FROM partii 
WHERE Algushetk BETWEEN '2005-03-04 09:00:00.000' 
AND '2005-03-04 11:00:00.000';
SELECT Valge, Must FROM partii 
WHERE MONTH(Algushetk)=3 
AND DAY(Algushetk)=4 
AND CONVERT(TIME,Algushetk) BETWEEN '09:00' AND '11:00'
...
SELECT* FROM isik 
WHERE EXISTS(SELECT* FROM partii 
WHERE isik.Id= partii.valge 
AND Datediff(minute, Algushetk, Lopphetk) BETWEEN 9 AND 11 AND Valge_tulemus=2);
...
SELECT COUNT(*) FROM partii 
WHERE EXISTS(SELECT* FROM isik 
...
SELECT COUNT(*) AS 'Arvod valgetega',partii.Turniir FROM partii 
WHERE EXISTS(SELECT* FROM isik 
WHERE partii.Valge= isik.id 
AND isik.Eesnimi='Arvo') 
GROUP BY partii.Turniir;
--või ilusate turniiride nimedega
SELECT turniir.nimi, COUNT(*) AS 'Arvod valgetega' FROM partii KEY JOIN turniir 
WHERE EXISTS(SELECT* FROM isik 
WHERE partii.Valge= isik.id 
AND isik.Eesnimi='Arvo') 
GROUP BY nimi;
...
SELECT SUM(CASE 
WHEN Musta_tulemus=2 THEN 1 
WHEN Musta_tulemus=1 THEN 0.5 
ELSE Musta_tulemus END) musta_tulemus 
FROM partii 
WHERE EXISTS(SELECT* FROM isik 
...
SELECT turniir.nimi AS 'turniiri nimi', AVG(Datediff(minute, ALgushetk, Lopphetk)) AS 'keskmine partii pikkus' FROM partii 
KEY JOIN turniir 
GROUP BY nimi;