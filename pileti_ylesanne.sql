...
--algne versioon, turniiri 47 kõige rohkem aeg kulutanud isiku nimi
...
select valge as isik, sum(datediff(minute, algushetk, lopphetk)) as aeg from partii where turniir=turniir_id group by isik
union
select must as isik, sum(datediff(minute, algushetk, lopphetk)) as aeg from partii where turniir=turniir_id group by isik
) as tabel join isik on isik.id=tabel.isik
group by nimi order by sum(aeg) desc;
RETURN nimi;
END;