...
RESULT (regulatsioon_link VARCHAR(300), arv INTEGER, moodik VARCHAR(100))
BEGIN
SELECT regulatsioon_link, SUM(arv) AS summa, moodik FROM (
select regulatsioon_link, COUNT(*) AS arv, 'osutamistearv' AS moodik from moodik 
join teenus_has_regulatsioon on moodik.teenus=teenus_has_regulatsioon.teenus_id 
where osutamistearv is not null
group by regulatsioon_link
union 
select regulatsioon_link, COUNT(*)as Arv, 'rahulolu' AS moodik from moodik 
join teenus_has_regulatsioon on moodik.teenus=teenus_has_regulatsioon.teenus_id 
where rahulolu is not null
group by regulatsioon_link
union 
select regulatsioon_link, COUNT(*) as Arv, 'ajakulu' AS moodik from moodik 
join teenus_has_regulatsioon on moodik.teenus=teenus_has_regulatsioon.teenus_id 
where ajakulu is not null
group by regulatsioon_link
union 
select regulatsioon_link, COUNT(*) as Arv, 'maksumus' AS moodik from moodik 
join teenus_has_regulatsioon on moodik.teenus=teenus_has_regulatsioon.teenus_id 
where maksumus is not null
group by regulatsioon_link) AS tabel
GROUP BY regulatsioon_link, moodik
ORDER by summa desc;
END
--näide
